...
class FullFusionDataset(Dataset):
    def __init__(self, x_paths,input_dim=544, target_len=550):  
        self.samples = []
        self.labels = []
        self.nature_flags = []
        self.subjs=[]
        ...
        for x_path in x_paths:
            ...
            if os.path.exists(p_path):
                p_data = np.load(p_path).astype(np.float32).reshape(-1, 1)
            else:
                p_data = np.zeros((x_data.shape[0], 1), dtype=np.float32)
            ...
            fused_data = np.hstack([x_data, p_data]) 
            ...
            if os.path.exists(y_path):
                ...
                raw_seq = np.zeros(min_len, dtype=np.float32)
            ...
            if curr_x > target_len:
                ...
                fused_data = np.vstack([fused_data, np.zeros((pad_amt, input_dim), dtype=np.float32)])
            ...
            if curr_y > target_len:
                ...
                raw_seq = np.pad(raw_seq, (0, pad_amt), mode='constant')
                
         
                
            self.samples.append(fused_data)  
            self.labels.append(label_seq)
            self.nature_flags.append(is_nature)
            self.raw_labels.append(raw_seq)
    ...
    def __getitem__(self, idx):  
        return (torch.from_numpy(self.samples[idx]), 
                torch.tensor(self.labels[idx], dtype=torch.long), torch.tensor(self.raw_labels[idx], dtype=torch.float32),
                self.nature_flags[idx],self.subjs[idx],self.vids[idx])
...
def run_full_fusion_experiment(data_folder, input_dim=544, hidden_dim=128, num_classes=2, 
    ...
    print(f"Starting FULL FUSION Experiment (Video+Audio+PPG) on {len(subjects)} subjects...")
    print(f"Input Dimension: {input_dim} (512 ResNet + 31 Audio + 1 PPG)")
    ...
    for fold in range(4):
        ...
        print("Test subjects:", test_sub_pool,"length:",len(test_sub_pool))
        print("Val subjects:",inner_val_subs,"length:",len(inner_val_subs))
        ts=set(train_pool_subjects)-set(inner_val_subs)
        print("Train subjects:",ts,"length:",len(ts))
        print()
        ...
        train_ds = FullFusionDataset(inner_train_files,input_dim=input_dim,target_len=target_len) 
        val_ds = FullFusionDataset(inner_val_files,input_dim=input_dim, target_len=target_len)
        test_ds = FullFusionDataset(test_files, input_dim=input_dim,target_len=target_len)
        
        train_loader = DataLoader(train_ds, batch_size=batch_size, shuffle=True) 
        val_loader = DataLoader(val_ds, batch_size=batch_size, shuffle=False)
        test_loader = DataLoader(test_ds, batch_size=batch_size, shuffle=False)
        
        model = FusionPredictor(input_dim, hidden_dim, num_classes).to(device)
        ...
        print(f"  > Class Imbalance: Non-Tingle={class_counts[0]}, Tingle={class_counts[1]}")
        ...
        for epoch in range(epochs):
            model.train()
            for X_b, y_b,_, _,_,_ in train_loader:
                X_b, y_b = X_b.to(device), y_b.to(device)
                optimizer.zero_grad()                          
                out = model(X_b)
                
                loss = criterion(out.transpose(1, 2), y_b) 
            ...
            preds, truths = [], []
            with torch.no_grad():   
                for X_b, y_b,_, _,_,_ in val_loader:
                    X_b, y_b = X_b.to(device), y_b.to(device)
                    out = model(X_b)    
        ...
        model.eval()
        with torch.no_grad():
            for X_b, y_b,raw_y_batch, is_nat_batch, subj_batch,vid_batch in test_loader:
                X_b = X_b.to(device)
                out = model(X_b)
                batch_preds = torch.argmax(out, dim=2).cpu().numpy()
                batch_truths = y_b.numpy()
                batch_probs = torch.softmax(out, dim=2)[:, :, 1].cpu().numpy()
                batch_raw_truths = raw_y_batch.numpy()
                
                for i in range(len(is_nat_batch)):
                    global_test_results.append({
                        'subj': subj_batch[i].item(),'vid_name': vid_batch[i],
                        'truths': batch_truths[i],
                        'preds': batch_preds[i],'probs': batch_probs[i],            
                        'raw_truths': batch_raw_truths[i],
                        'is_nature': is_nat_batch[i].item()
                    })
    ...
    print()
    print(f"      FINAL FUSION SUMMARY (4-FOLD)      ")
    print()
...
if __name__ == "__main__":
    results = run_full_fusion_experiment(data_folder="./data/features")