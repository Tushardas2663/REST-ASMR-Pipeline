...
class AudioOnlyDataset(Dataset):
    def __init__(self, x_paths,input_dim=31, target_len=550):  
        self.samples = []
        self.labels = []
        self.nature_flags = []
        self.subjs=[]
        ...
        for x_path in x_paths:
            ...
            fused_data = x_data[:, 512:543]
            min_len = len(fused_data)
    ...
    def __getitem__(self, idx): 
        return (torch.from_numpy(self.samples[idx]), 
                ...
                self.nature_flags[idx], self.subjs[idx])
...
def run_audio_only_experiment(data_folder, input_dim=31, hidden_dim=128, num_classes=2, 
    ...
    print(f"Starting AUDIO ONLY Experiment on {len(subjects)} subjects...")
    print(f"Input Dimension: {input_dim} (31 Audio Features)")
    ...
    for fold in range(4):
        ...
        train_ds = AudioOnlyDataset(inner_train_files, input_dim=input_dim,target_len=target_len) 
        val_ds = AudioOnlyDataset(inner_val_files,input_dim=input_dim,target_len=target_len)
        test_ds = AudioOnlyDataset(test_files,input_dim=input_dim,target_len=target_len)
        ...
        model.eval()
        with torch.no_grad():
            for X_b, y_b, is_nat_batch, subj_batch in test_loader:
                X_b = X_b.to(device)
                out = model(X_b)
    ...
    print()
    print(f"      FINAL AUDIO-ONLY SUMMARY (4-FOLD)      ")
    print()
...
if __name__ == "__main__":
    results = run_audio_only_experiment(data_folder="./data/features")
